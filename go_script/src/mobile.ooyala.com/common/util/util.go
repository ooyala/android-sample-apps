...
func Die(err error, logger *log.Logger) {
	if err != nil {
		...
		if logger != nil {
			ol.ColorizedPrintln(logger, msg)
		} else {
...
func RunBashCommand(command string, logger *log.Logger) (string, error) {
	...
	if err != nil {
		...
		err = NewErrorWithMessageAndErrorAndStack( msg, err )
	}
...
func MakeCountComparisonCommand(srcFindCmd string, dstFindCmd string, comparison string) string {
	return "if [ `" + srcFindCmd + " | wc -l` " + comparison + " `" + dstFindCmd + " | wc -l` ]; then exit 0; else exit 1; fi"
}

func RequireEqualCount(runPath DirAbs, srcFindCmd string, dstFindCmd string, logger *log.Logger) error {
	cmd := MakeCountComparisonCommand(srcFindCmd, dstFindCmd, "-eq")
	_, err := RunBashCommandInDir(runPath, cmd, logger)
	return err
}

func RequireGreaterThanOrEqualCount(runPath DirAbs, srcFindCmd string, dstFindCmd string, logger *log.Logger) error {
	cmd := MakeCountComparisonCommand(srcFindCmd, dstFindCmd, "-le") // the source count should be <= destination count.
	_, err := RunBashCommandInDir(runPath, cmd, logger)
	return err
}
